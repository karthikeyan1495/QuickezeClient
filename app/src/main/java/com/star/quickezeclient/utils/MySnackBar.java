package com.star.quickezeclient.utils;
...
/*    private void showpositiveBar(Activity activity,String message, String color){
        TSnackbar snackbar = TSnackbar.make(activity.findViewById(android.R.id.content), message, TSnackbar.LENGTH_LONG);
        snackbar.setMaxWidth(Util.getInstance().getScreenWidth(activity));
        View snackbarView = snackbar.getView();
        snackbarView.setBackgroundColor(Color.parseColor("#2BAB08"));
        TextView textView = (TextView) snackbarView.findViewById(com.androidadvance.topsnackbar.R.id.snackbar_text);
        textView.setTextColor(Color.parseColor(color));
        textView.setTextSize(17);
        snackbar.show();
